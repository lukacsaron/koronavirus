...
fig_confirmed.add_trace(go.Scatter(x=df_confirmed['Date'], y=df_confirmed['Mainland China'],
                                   ...
                                   hovertext=['Visszaigazolt esetek<br>{:,d} Kínában<br>'.format(i) for i in df_confirmed['Mainland China']],
...
fig_confirmed.add_trace(go.Scatter(x=df_confirmed['Date'], y=df_confirmed['Other locations'],
                                   ...
                                   text=[datetime.strftime(d, '%b %d %Y AEDT') for d in df_confirmed['Date']],
                                   hovertext=['Visszaigazolt esetek<br>{:,d} Kínán kívül<br>'.format(i) for i in df_confirmed['Other locations']],
                                   hovertemplate='<b>%{text}</b><br></br>'+
                                                 '%{hovertext}'+
                                                 '<extra></extra>'))
# Customise layout
...
fig_combine.add_trace(go.Scatter(x=df_recovered['Date'], y=df_recovered['Total'],
                                   ...
                                   hovertext=['Összes felépült<br>{:,d} eset<br>'.format(i) for i in df_recovered['Total']],
                                   hovertemplate='<b>%{text}</b><br></br>'+
                                                 '%{hovertext}'+
                                                 '<extra></extra>'))
...
fig_rate.add_trace(go.Scatter(x=df_deaths['Date'], y=df_deaths['Mainland China']/df_confirmed['Mainland China']*100,
                                ...
                                marker=dict(size=6, color='#f4f4f2',
                                            line=dict(width=1,color='#626262')),
                                text=[datetime.strftime(d, '%b %d %Y AEDT') for d in df_deaths['Date']],
                                hovertext=['Halálesetek aránya Kínában<br>{:.2f}%'.format(i) for i in df_deaths['Mainland China']/df_confirmed['Mainland China']*100],
                                hovertemplate='<b>%{text}</b><br></br>'+
                                              '%{hovertext}'+
                                              '<extra></extra>'))
fig_rate.add_trace(go.Scatter(x=df_deaths['Date'], y=df_deaths['Other locations']/df_confirmed['Other locations']*100,
                                ...
                                text=[datetime.strftime(d, '%b %d %Y AEDT') for d in df_deaths['Date']],
                                hovertext=['Halálesetek aránya Kínán kívül<br>{:.2f}%'.format(i) for i in df_deaths['Other locations']/df_confirmed['Other locations']*100],
                                hovertemplate='<b>%{text}</b><br></br>'+
                                              '%{hovertext}'+
                                              '<extra></extra>'))
...
app.layout = html.Div(style={'backgroundColor':'#f4f4f2'},
    children=[
        html.Div(
            id="header",
            children=[                          
                html.H4(children="Koronavírus (COVID-19) Járványmonitor"),
                html.P(
                    id="description",
                    children="2020. január elején egy eddig ismeretlen koronavírus okozott felső légúti megbetegedéseket és tüdőgyulladást Kínában, a 12 millió lakosú Vuhan városában, aztán átterjedt más tartományokra is, azóta pedig világjárvány tört ki. \
                    Ez a infomonitor azért készült, hogy valós időben követhesse a járvány terjedését.".format(latestDate, confirmedCases),
                ),
 #              html.P(
 #               	id="note",
 ...
 #               ),
                ...
                html.P(style={'fontWeight':'bold'},
                       children="Utoljára frissítve {}.".format(latestDate))
                    ]        
                ),
        html.Div(
            id="number-plate",
                         ...
                         style={'width':'24.4%','backgroundColor':'#cbd2d3','display':'inline-block',
                              ...
                              children=[
                                  ...
                                  html.H5(style={'textAlign':'center','color':'#2674f6','padding':'.1rem'},
                                               children="Nap telt el a járvány kezdete óta")                                        
                                       ]),
                     html.Div(
                         style={'width':'24.4%','backgroundColor':'#cbd2d3','display':'inline-block',
                                'marginRight':'.8%','verticalAlign':'top'},
                                  ...
                                  html.H3(style={'textAlign':'center',
                                                ...
                                                children=[
                                                    html.P(style={'padding':'.5rem'},
                                                              children='+ {:,d} az elmúlt 24 órában ({:.1%})'.format(plusConfirmedNum, plusPercentNum1)),
                                                    '{:,d}'.format(confirmedCases)
                                                         ]),
                                  html.H5(style={'textAlign':'center','color':'#d7191c','padding':'.1rem'},
                                               children="Visszaigazolt Esetszám")                                        
                                       ]),
                     html.Div(
                         style={'width':'24.4%','backgroundColor':'#cbd2d3','display':'inline-block',
                                'marginRight':'.8%','verticalAlign':'top'},
                              children=[
                                  html.H3(style={'textAlign':'center',
                                               ...
                                               children=[
                                                   html.P(style={'padding':'.5rem'},
                                                              children='+ {:,d} az elmúlt 24 órában ({:.1%})'.format(plusRecoveredNum, plusPercentNum2)),
                                                   '{:,d}'.format(recoveredCases),
                                               ]),
                                  html.H5(style={'textAlign':'center','color':'#1a9622','padding':'.1rem'},
                                               children="Felépült Esetszám")                                        
                                       ]),
                     html.Div(
                         style={'width':'24.4%','backgroundColor':'#cbd2d3','display':'inline-block',
                                'verticalAlign':'top'},
                              children=[
                                  html.H3(style={'textAlign':'center',
                                                ...
                                                children=[
                                                    html.P(style={'padding':'.5rem'},
                                                              children='+ {:,d} az elmúlt 24 órában ({:.1%})'.format(plusDeathNum, plusPercentNum3)),
                                                    '{:,d}'.format(deathsCases)
                                                ]),
                                  html.H5(style={'textAlign':'center','color':'#6c6c6c','padding':'.1rem'},
                                               children="Halálos Esetszám")                                        
                                       ])
                          ]),
        html.Div(
            id='dcc-plot',
                     ...
                     html.Div(
                         style={'width':'32.79%','display':'inline-block','marginRight':'.8%','verticalAlign':'top'},
                              children=[
                                  html.H5(style={'textAlign':'center','backgroundColor':'#cbd2d3',
                                                 'color':'#292929','padding':'1rem','marginBottom':'0'},
                                               children='Felépültek/Halálesetek'),
                 ...
                 children=[
                     ...
                     html.Div(style={'width':'32.79%','display':'inline-block','verticalAlign':'top'},
                              children=[
                                  html.H5(style={'textAlign':'center','backgroundColor':'#cbd2d3',
                                                 'color':'#292929','padding':'1rem','marginBottom':'0'},
                                               children='Esetek Ország Szerint'),
